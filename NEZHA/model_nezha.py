    ...
    def __init__(self,
        ...
        Args:
            ...
            attention_probs_dropout_prob: The dropout ratio for the attention
                probabilities.
            max_position_embeddings: The maximum sequence length that this save_model might
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            raise ValueError("First argument must be either a vocabulary size (int)"
                             "or the path to a pretrained save_model config file (str)")
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        if not isinstance(config, BertConfig):
            raise ValueError(
                ...
                "To create a save_model from a Google pretrained save_model use "
                "`save_model = {}.from_pretrained(PRETRAINED_MODEL_NAME)`".format(
...
class BertForPreTraining(BertPreTrainedModel):
    """BERT save_model with pre-training heads.
    This module comprises the BERT save_model followed by the two pre-training heads:
        - the masked language modeling head, and
        - the next sentence classification head.

    Params:
        config: a BertConfig class instance with the configuration to build a new save_model.
    ...
    save_model = BertForPreTraining(config)
    masked_lm_logits_scores, seq_relationship_logits = save_model(input_ids, token_type_ids, input_mask)
    ```
    """
...
class BertForMaskedLM(BertPreTrainedModel):
    """BERT save_model with the masked language modeling head.
    This module comprises the BERT save_model followed by the masked language modeling head.
    ...
    save_model = BertForMaskedLM(config)
    masked_lm_logits_scores = save_model(input_ids, token_type_ids, input_mask)
    ```
    """
...
class BertForSequenceClassification(BertPreTrainedModel):
    """BERT save_model for classification.
    This module is composed of the BERT save_model with a linear layer on top of
    the pooled output.
    ...
    save_model = BertForSequenceClassification(config, num_labels)
    logits = save_model(input_ids, token_type_ids, input_mask)
    ```
    """
