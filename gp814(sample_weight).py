...
#%%
datalist = []
with open('./datasets/preliminary_contest_datasets/train_data/train+pseudo_data_5w.txt', 'r', encoding='utf-8') as f:
    lines = f.readlines()
    lines.append('\n')
...
def validation_fn(model, dev_loader, loss_fn):
    ...
    with torch.no_grad():
        for token_id, at_mask, label_id, token_type_ids, sample_weight in dev_loader:
...
def train_model(model, train_loader, dev_loader, model_save_path='../outputs',
    ...
    for epoch in range(EPOCHS):
        ...
        for idx, (token_id, at_mask, label_id, token_type_ids, sample_weight) in enumerate(bar):
            outputs = model(token_id.to(device), at_mask.to(device), token_type_ids.to(device))
            loss = loss_fn(outputs, label_id.to(device), sample_weight.to(device))
            total_loss.append(loss.item())
            loss.backward()
