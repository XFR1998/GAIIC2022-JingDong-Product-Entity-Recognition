...
def save_vocab(sents):
    ...
    word2id['unk'] = i + 1
    id2word[i + 1] = 'unk'
    return word2id, id2word, len(word2id) + 1


word2id, id2word, vocab_size = save_vocab(pd.DataFrame(datalist)['text'])
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        token_word = self.cut_word(row)
        return torch.tensor(token_ids, dtype=torch.long), \
               torch.tensor(at_mask, dtype=torch.long), \
               torch.tensor(self.ark_data[index]['label_ids'].to_dense()), \
               torch.tensor(self.ark_data[index]['token_type_ids'], dtype=torch.long), \
               torch.tensor(token_word, dtype=torch.long)
    ...
    def cut_word(self, row):
        sentence = row.text
        word_list = list(jieba.cut(sentence, cut_all=True))
        token_list = ['[CLS]'] + self.tokenizer.tokenize(sentence)
        ...
        while len(char_word) < self.max_len:
            char_word.append([0 for i in range(4)])
        return char_word[:self.max_len]
    ...
    def collate_fn(self, batch):
        ...
        token_word = torch.stack([x[4] for x in batch])
        return token_ids, at_mask, labels.squeeze(), token_type_ids, token_word
...
class GlobalPointer(nn.Module):
    def __init__(self, encoder, ent_type_size, inner_dim, RoPE=True, dropout_rate=0.5):
        super().__init__()
        self.encoder = encoder
        self.ent_type_size = ent_type_size
        self.inner_dim = inner_dim
        self.hidden_size = encoder.config.hidden_size
        self.dense = nn.Linear(self.hidden_size * 3, self.ent_type_size * self.inner_dim * 2)
        ...
        self.word_embeddings = nn.Embedding(vocab_size, self.hidden_size)
        self.word_layer1 = torch.nn.Sequential(
            torch.nn.Linear(in_features=self.hidden_size, out_features=1024),
            torch.nn.ReLU(),
            torch.nn.Dropout(p=dropout_rate))
        self.word_layer2 = torch.nn.Linear(in_features=1024, out_features=self.hidden_size)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, token_word=None):
        ...
        word_emb = torch.sum(word_emb, dim=2).view(-1, seq_len, self.hidden_size)

        last_hidden_state = torch.cat([last_hidden_state, word_emb], dim=-1)
...
def validation_fn(model, dev_loader, loss_fn):
    ...
    with torch.no_grad():
        for token_id, at_mask, label_id, token_type_ids, token_word in dev_loader:
            outputs = model(token_id.to(device), at_mask.to(device),
                            token_type_ids.to(device), token_word.to(device))
...
def train_model(model, train_loader, dev_loader, model_save_path='../outputs',
    ...
    for epoch in range(EPOCHS):
        ...
        for idx, (token_id, at_mask, label_id, token_type_ids, token_word) in enumerate(bar):
            outputs = model(token_id.to(device), at_mask.to(device),
                            token_type_ids.to(device), token_word.to(device))
            loss = loss_fn(outputs, label_id.to(device))
            total_loss.append(loss.item())
        ...
        model_save_path = 'model/nezha_sl_test_model.pt'
...
class GlobalPointerNERPredictor(object):
    ...
    def _convert_to_transfomer_ids(
        ...
        word_list = list(jieba.cut(text, cut_all=True))
        token_list = ['[CLS]'] + self.tokenizer.tokenize(text)
        token_list = [token.replace('##', '') for token in token_list]
        win = ['。' for i in range(6)]
        token_list = win + token_list + win
                ...
                if token == word:
                    ...
                    if index == 0:
                        if word in word2id:
                            ...
                            char_word_id[0] = word2id['unk']
                    elif index == len(word) - 1:
                        if word in word2id:
                            char_word_id[2] = word2id[word]
                        else:
                            char_word_id[2] = word2id['unk']
                    else:
                        if word in word2id:
                            char_word_id[1] = word2id[word]
                        else:
                            char_word_id[1] = word2id['unk']
            char_word.append(char_word_id)
        while len(char_word) < self.tokenizer.max_seq_len:
            char_word.append([0 for i in range(4)])
        char_word = char_word[:self.tokenizer.max_seq_len]
    ...
    def predict_one_sample(
        ...
        with torch.no_grad():
            ...
            token_word = inputs['token_word']

            scores = self.module(input_ids, attention_mask, token_type_ids, token_word)[0].cpu()
...
model = GlobalPointer(encoder, len(Ent2id), 64)  # (encoder, ent_type_size, inner_dim)
model = model.to(device)
path = 'model/nezha_sl_test_model.pt'
...
with open('../preliminary_test_b/sample_per_line_preliminary_B.txt', 'r',
          encoding='utf-8') as f:
    lines = f.readlines()
