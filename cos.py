# -*- coding: utf-8 -*-
"""
Created on Sun May  8 13:41:03 2022
...
class LR_Scheduler(object):
    def __init__(self, optimizer,warmup_iter,total_steps,warmup_lr,base_lr, final_lr):
...
#对warmup进行设置
#注意，这里使用了梯度累计，我们相当于是32，因为warmup是基于step进行更新的
warmup_iter = 563#这里取得是总步长的前10%
    ...
    for epoch in range(EPOCHS):
        ...
        for idx, (token_id, at_mask, label_id, token_type_ids) in enumerate(bar):
            ...
            if ((idx + 1) % 4) == 0:
                ...
                lr_scheduler.step()
                list1.append(lr_scheduler.get_lr())
        ...
        model_save_path = 'model/nezha_test_model.pt'
...
model = GlobalPointer(encoder, len(Ent2id), 64)  # (encoder, ent_type_size, inner_dim)
model = model.to(device)
path = 'model/nezha_test_model.pt'
